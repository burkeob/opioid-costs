...
### Introduction
This project distributes national estimates of the costs of the opioid epidemic among U.S. states and counties. We update aggregate cost measures come from the CEA report "The Underestimate Cost of the Opioid Crisis" (2017) to reflect the most recent data. We break down our $765 billion estimate for 2017 into non-mortality and mortality related costs as follows: `r knitr::kable(costdt)`
...
drug_deaths_mcd <- read_tsv("../data/Drug Deaths 2013_2017 - MCD.txt") %>%
...
gpci_level_2018 <- read_csv("../data/gpci_2018.csv") %>%
  ...
  mutate(GPCI18 = 4.5 * `GPCI WORK` + 1.42 * `GPCI PE` + 0.38 * `GPCI MP`) %>%
...
gpci_level_2019 <- read_csv("../data/gpci_2019.csv") %>%
  select(`MAC LOCALITY`, `GPCI WORK`, `GPCI PE`, `GPCI MP`) %>%
  mutate(GPCI19 = 4.5 * `GPCI WORK` + 1.42 * `GPCI PE` + 0.38 * `GPCI MP`) %>%
  right_join(gpci_xwalk) %>%
  mutate(State = ifelse(State == "HAWAII/GUAM", "HAWAII", State)) %>%
  filter(!is.na(GPCI19)) %>%
  unique %>%
...
gpci <- gpci_level_2019 %>% full_join(gpci_level_2018)

gpci_model <- lm(GPCI19 ~ GPCI18, data = gpci)

gpci <- gpci %>% 
  mutate(predicted_gpci19 = predict(gpci_model, gpci %>% select(-GPCI19), allow.new.levels=TRUE, type = "response"))

gpci <- gpci %>% 
  mutate(GPCI = ifelse(!is.na(GPCI19), GPCI19, predicted_gpci19)) %>%
  select(-GPCI19, -GPCI18, -predicted_gpci19)
...
counts_2017 <- read_xlsx("../Data/NSDUHsaeTotals2017.xlsx", sheet = "Table 21", skip = 7)
names(counts_2017) <- str_remove_all(names(counts_2017), "\r|\n")
...
hospitalization_costs16 <- read_rds("../data/new_hospitalization_costs.rds") %>% 
  rename(total_costs16 = total_costs) %>%
  select(region_name, state_name, total_costs16)
hospitalization_costs14 <- read_rds("../data/hospitalization_costs.rds") %>% rename(total_costs14 = total_costs) %>%
  select(region_name, state_name, total_costs14)

# county-level data
county_hosp_costs <- hospitalization_costs16 %>% full_join(hospitalization_costs14) %>% filter(region_name != "State Total") %>%
  mutate(county.name = region_name %>% tolower %>% str_trim,
         state.name = state_name %>% tolower %>% str_trim) %>%
  inner_join(county.regions) 


county_hosp_model <- glmmTMB(total_costs16 ~ total_costs14 + (1|state.fips.character), data = county_hosp_costs)

county_hosp_costs <- county_hosp_costs %>% 
  mutate(predicted_costs16 = predict(county_hosp_model, county_hosp_costs %>% select(-total_costs16), allow.new.levels=TRUE, type = "response"))

county_hosp_costs <- county_hosp_costs %>% 
  mutate(total_costs = ifelse(!is.na(total_costs16), total_costs16, predicted_costs16)) 
# state-level data
state_hosp_costs16 <- read_rds("../data/new_hospitalization_costs.rds") %>% filter(region_name == "State Total") %>%
  mutate(region = state_name %>% tolower %>% str_trim,
         state_total_costs16 = total_costs) %>%
  select(region, state_total_costs16)

state_hosp_costs14 <- read_rds("../data/hospitalization_costs.rds") %>% filter(region_name == "State Total") %>%
  mutate(region = state_name %>% tolower %>% str_trim,
         state_total_costs14 = total_costs) %>%
  select(region, state_total_costs14)

state_hosp_costs <- state_hosp_costs16 %>% full_join(state_hosp_costs14)

state_hosp_model <- lm(state_total_costs16 ~ state_total_costs14, data = state_hosp_costs)

state_hosp_costs <- state_hosp_costs %>% 
  mutate(predicted_state_costs16 = predict(state_hosp_model, state_hosp_costs %>% select(-state_total_costs16), allow.new.levels=TRUE, type = "response"))

state_hosp_costs <- state_hosp_costs %>% 
  mutate(state_total_costs = ifelse(!is.na(state_total_costs16), state_total_costs16, predicted_state_costs16))

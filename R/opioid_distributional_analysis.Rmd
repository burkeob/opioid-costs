...
Overall drug-related deaths include those associated with the following 2016 ICD-10 Codes. For the drug-related death data, we capture all drug-related deaths from 2008 through 2017: 
...
opioid_deaths17 <- read_tsv("../data/Opioid Deaths 2017 - MCD.txt") %>%
  select(-Notes) %>% 
  filter(!is.na(County)) %>% 
  mutate(region = `County Code` %>% as.numeric,
         opioid_deaths17 = Deaths) %>%
  select(region, opioid_deaths17) 

opioid_deaths16 <- read_tsv("../data/Opioid Deaths 2016 - MCD.txt") %>%
  select(-Notes) %>% 
  filter(!is.na(County)) %>% 
  mutate(region = `County Code` %>% as.numeric,
         opioid_deaths16 = Deaths) %>%
  select(region, opioid_deaths16)

opioid_deaths <- opioid_deaths17 %>% full_join(opioid_deaths16) %>% 
  left_join(county.regions %>% select(region, state.fips.character))

opioid_deaths_model <- glmmTMB(opioid_deaths17 ~ opioid_deaths16 + (1|state.fips.character), data = opioid_deaths)

opioid_deaths <- opioid_deaths %>% 
  mutate(predicted_deaths17 = predict(opioid_deaths_model, opioid_deaths %>% select(-opioid_deaths17), allow.new.levels=TRUE, type = "response"))

opioid_deaths <- opioid_deaths %>% 
  mutate(opioid_deaths = ifelse(!is.na(opioid_deaths17), opioid_deaths17, predicted_deaths17)) %>%
...
drug_deaths_mcd16 <- read_tsv("../data/Drug Deaths 2016 - MCD.txt") %>%
  filter(!is.na(`County Code`)) %>%
  mutate(region = `County Code` %>% as.numeric,
         drug_deaths_mcd16 = Deaths) %>%
  select(region, drug_deaths_mcd16)

drug_deaths_mcd17 <- read_tsv("../data/Drug Deaths 2017 - MCD.txt") %>%
  filter(!is.na(`County Code`)) %>%
  mutate(region = `County Code` %>% as.numeric,
         drug_deaths_mcd17 = Deaths) %>%
  select(region, drug_deaths_mcd17)

# Combine drug death data
drug_deaths <- county.regions %>% 
  as_tibble %>% 
  select(region, state.fips.character) %>%
  full_join(drug_deaths_mcd16) %>%
  full_join(drug_deaths_mcd17)

drug_deaths_model <- glmmTMB(drug_deaths_mcd17 ~ drug_deaths_mcd16 + (1|state.fips.character), data = drug_deaths)

drug_deaths <- drug_deaths %>% 
  mutate(predicted_deaths17 = predict(drug_deaths_model, drug_deaths %>% select(-drug_deaths_mcd17), allow.new.levels=TRUE, type = "response"))

drug_deaths <- drug_deaths %>% 
  mutate(drug_deaths = ifelse(!is.na(drug_deaths_mcd17), drug_deaths_mcd17, predicted_deaths17)) %>%
  select(region, drug_deaths)
...
addiction_xwalk1 <- read_xlsx("../data/substate_county141516.xlsx") %>% tbl_df %>%
  select(sbst16n, state, county) %>%
...
addiction_xwalk2 <- read_xlsx("../data/substate_tract141516.xlsx") %>% tbl_df %>%
  select(sbst16n, state, county) %>% 
  group_by(sbst16n, state, county) %>%
...
nonmedical_use <- addiction_xwalk %>% rename(state_fips = state) %>% 
  mutate(sbst14n = case_when(sbst16n == "Trillium Health Resources 1" ~ "Trillium Healthcare Resources 1",
                            sbst16n == "Trillium Health Resources 2" ~ "Trillium Healthcare Resources 2",
                            TRUE ~ sbst16n)) %>% 
...
criminal_costs <- read_csv("../data/criminal_justice_expenditures_2016.csv",
              skip = 20, col_names = F) %>%
  na.omit %>%
  filter(X2 != "-") %>%
  select(X1, X2, X4) %>%
...
hospitalization_costs14 <- read_rds("../data/hospitalization_costs.rds")
hospitalization_costs16 <- read_rds("../data/new_hospitalization_costs.rds")

hospitalization_costs <- hospitalization_costs14 %>% full_join(hospitalization_costs16)
...
hosp_costs_model <- county_hosp_costs %>%  
  ...
  mutate(log_total_hosp_costs = log(total_costs),
         ...
         log_drug_deaths_mcd = log(drug_deaths))
